...
type x11Window struct {
	...
	atoms struct {
		...
		xdndFinished   C.Atom
		xdndStatus     C.Atom
		xdndSelection  C.Atom
		xdndActionCopy C.Atom
		xdndTypelist   C.Atom
		textURIList    C.Atom
	}

	// XDND state
	xdndSourceWin C.Window
	metric unit.Metric
	notify struct {
		read, write int
	}
...
func (h *x11EventHandler) handleEvents() bool {
	...
	for C.XPending(w.x) != 0 {
		...
		case C.SelectionNotify:
			...
			fmt.Printf("xdnd: SelectionNotify sel=0x%x prop=0x%x target=0x%x\n", cevt.selection, cevt.property, cevt.target)
			// XDND selection (file drop)
			if cevt.selection == w.atoms.xdndSelection {
				fmt.Printf("xdnd: → matched xdndSelection, calling handleXdndSelection\n")
				w.handleXdndSelection(cevt)
				break
			}
		...
		case C.ClientMessage: // extensions
			...
			switch cevt.message_type {
			case w.atoms.xdndEnter:
				w.xdndSourceWin = C.Window(*(*C.long)(unsafe.Pointer(&cevt.data)))
				fmt.Printf("xdnd: Enter source=0x%x\n", w.xdndSourceWin)
			case w.atoms.xdndPosition:
				w.xdndSourceWin = C.Window(*(*C.long)(unsafe.Pointer(&cevt.data)))
				// Reply with XdndStatus: accept drop, want more position events
				var reply C.XClientMessageEvent
				reply._type = C.ClientMessage
				reply.display = w.x
				reply.window = w.xdndSourceWin
				reply.message_type = w.atoms.xdndStatus
				reply.format = 32
				xdata := (*[5]C.long)(unsafe.Pointer(&reply.data))
				xdata[0] = C.long(w.xw)       // target window
				xdata[1] = 3                   // bit0=accept, bit1=want position
				xdata[2] = 0                   // rectangle origin
				xdata[3] = 0                   // rectangle size (0=always send)
				xdata[4] = C.long(w.atoms.xdndActionCopy)
				C.XSendEvent(w.x, w.xdndSourceWin, C.False, C.NoEventMask,
					(*C.XEvent)(unsafe.Pointer(&reply)))
				C.XFlush(w.x)
				fmt.Printf("xdnd: Position → Status target=0x%x source=0x%x\n", w.xw, w.xdndSourceWin)
			case w.atoms.xdndDrop:
				w.xdndSourceWin = C.Window(*(*C.long)(unsafe.Pointer(&cevt.data)))
				dropTS := C.Time((*(*[5]C.long)(unsafe.Pointer(&cevt.data)))[2])
				C.XConvertSelection(w.x, w.atoms.xdndSelection, w.atoms.textURIList,
					w.atoms.xdndSelection, w.xw, dropTS)
				C.XFlush(w.x)
				fmt.Printf("xdnd: Drop source=0x%x\n", w.xdndSourceWin)
			case w.atoms.xdndLeave:
				fmt.Printf("xdnd: Leave source=0x%x\n", C.Window(*(*C.long)(unsafe.Pointer(&cevt.data))))
				w.xdndSourceWin = 0
			default:
...
func newX11Window(gioWin *callbacks, options []Option) error {
	...
	// XDND atoms
	w.atoms.xdndAware = w.atom("XdndAware", false)
	w.atoms.xdndEnter = w.atom("XdndEnter", false)
	w.atoms.xdndPosition = w.atom("XdndPosition", false)
	w.atoms.xdndDrop = w.atom("XdndDrop", false)
	w.atoms.xdndLeave = w.atom("XdndLeave", false)
	w.atoms.xdndFinished = w.atom("XdndFinished", false)
	w.atoms.xdndStatus = w.atom("XdndStatus", false)
	w.atoms.xdndSelection = w.atom("XdndSelection", false)
	w.atoms.xdndActionCopy = w.atom("XdndActionCopy", false)
	w.atoms.xdndTypelist = w.atom("XdndTypeList", false)
	w.atoms.textURIList = w.atom("text/uri-list", false)

	// Set XdndAware on the window (XDND version 5)
	xdndVer := C.long(5)
	C.XChangeProperty(dpy, win, w.atoms.xdndAware, C.XA_ATOM, 32,
		C.PropModeReplace, (*C.uchar)(unsafe.Pointer(&xdndVer)), 1)
...
// handleXdndSelection processes the selection data after XdndDrop.
func (w *x11Window) handleXdndSelection(cevt *C.XSelectionEvent) {
	var raw string
	if cevt.property != C.None {
		var actualType C.Atom
		var actualFormat C.int
		var nitems, bytesAfter C.ulong
		var data *C.uchar
		C.XGetWindowProperty(w.x, w.xw, cevt.property, 0, 65536, C.True, C.AnyPropertyType,
	...
	if w.xdndSourceWin != 0 {
		var fin C.XClientMessageEvent
		fin._type = C.ClientMessage
		fin.display = w.x
		fin.window = w.xdndSourceWin
		fin.message_type = w.atoms.xdndFinished
		fin.format = 32
		finData := (*[5]C.long)(unsafe.Pointer(&fin.data))
		finData[0] = C.long(w.xw)
		if len(raw) > 0 {
			finData[1] = 1
			finData[2] = C.long(w.atoms.xdndActionCopy)
		}
		C.XSendEvent(w.x, w.xdndSourceWin, C.False, C.NoEventMask,
	...
	fmt.Printf("xdnd: selection raw=%q (len=%d)\n", raw, len(raw))
	if len(raw) > 0 {
		select {
		case FileDropChan <- raw:
			fmt.Printf("xdnd: sent to FileDropChan\n")
		default:
			fmt.Printf("xdnd: FileDropChan full, dropping\n")
		}
	}
}
