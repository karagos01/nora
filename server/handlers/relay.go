...
func (d *Deps) RelayEvent(w http.ResponseWriter, r *http.Request) {
	...
	var signedData []byte
	if req.Type == "dm" {
		signedData = []byte(req.Type + ":" + req.EncryptedContent)
	} else {
		signedData = []byte(req.Type + ":" + string(req.Payload))
	}
	...
	if len(req.Type) >= 5 && req.Type[:5] == "call." {
		d.handleRelayCall(w, recipient, req.FromPublicKey, req.Type, req.Payload)
	} else {
		util.Error(w, http.StatusBadRequest, "unsupported relay type (supported: call.*)")
	}
}
